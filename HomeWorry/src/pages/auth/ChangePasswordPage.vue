<template>
  <div>
    ...
    <!-- 새 비밀번호 -->
      ...
      <!-- 버튼 하단 고정 -->
      <BtnMed class="submit-btn" type="submit" text="비밀번호 재설정하기" :disabled="loading"/>
    </form>
  </div>
</template>
...
async function onSubmit() {
  ...
  if (!passwordRegex.test(password.value)) {
    formError.value = '영문, 숫자, 특수문자 중 2종류 이상을 조합해 8자 이상 입력하세요.'
    loading.value = false
    return
  }
  ...
  delete axios.defaults.headers.common['Authorization']
    ...
    formError.value = err.response?.data?.message || '비밀번호 재설정에 실패했습니다.<br>잠시 후 다시 시도해주세요.'
  } finally {
    loading.value = false
  }
}
