<template>
  <div style="width: 100%; height: 100vh">
    ...
    <div class="flex gap-4 p-4">
      <div :class="randomBgClass" class="bg-opacity-30 p-2 rounded">
        ...
        <div class="flex items-center gap-2 mt-2 p-2 bg-gray-100 rounded">
          ...
          <span class="text-sm"> 평수: {{ minPyeong || '전체' }} ~ {{ maxPyeong || '전체' }} </span>
          <span class="text-sm"> | 거래유형: {{ selectedTransactionType || '전체' }} </span>
    ...
    >
      ...
      <template v-if="level >= 4" v-for="(dongMarker, index) in dongMarkers" :key="index">
        <KakaoMapCustomOverlay :lat="dongMarker.lat" :lng="dongMarker.lng" :y-anchor="1.4">
      ...
      <template v-if="level < 4" v-for="(marker, index) in markers" :key="index">
    ...
    <h1 class="dong-label bg-opacity-50" :class="randomBgClass">
      📍 {{ currentDong }} ({{ mapCenter.lat.toFixed(4) }}, {{ mapCenter.lng.toFixed(4) }})
      {{ level }}
    </h1>
  </div>
</template>
...
function handleTransactionTypeUpdate(type) {
