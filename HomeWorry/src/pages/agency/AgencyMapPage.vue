        ...
        <template v-if="level <= 4">
          <template v-for="marker in agencies" :key="marker.officeId">
            <KakaoMapMarker
              ...
              :lng="marker.lng"
              :clickable="true"
              :image="{ imageSrc: pinSrc, imageWidth: 36, imageHeight: 36 }"
              @onClickKakaoMapMarker="() => onMarkerClick(marker)"
            />

            <!-- 선택된 항목만 커스텀 오버레이 -->
            <template v-if="selectedAgency && selectedAgency.officeId === marker.officeId">
              <KakaoMapCustomOverlay :lat="marker.lat" :lng="marker.lng" :z-index="9999">
                <AgencyMapCard
