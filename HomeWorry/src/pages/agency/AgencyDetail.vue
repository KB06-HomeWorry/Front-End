    ...
    <div class="agency-profile-wrap">
      <div class="profile-row">
        ...
        <div class="profile-info">
          ...
          <div class="hashtags">
            <Hashtag v-for="tag in agency.hashtags" :key="tag">{{
              tag
            }}</Hashtag>
          </div>
        </div>
      ...
      <div class="agency-description">
        <div class="desc-content bodyLight12px">
          {{ agency.description || "등록된 소개글이 없습니다." }}
        </div>
      </div>
...
async function fetchFavoriteStatus() {
  try {
    const res = await axios.get(
      `/api/agent/${userToken}/isFavorite/${office_id}`
    );
...
onMounted(async () => {
  try {
    // 중개사 정보 조회
    const res = await axios.get(
      `http://54.66.153.95:8080/api/agent/${office_id}`
    );
    agency.value = {
      ...agency.value,
    ...
    // 중개사 신뢰점수 조회
    const res_score = await axios.get(
      `http://54.66.153.95:8080/api/agent/trustScore/${office_id}`
    );
    agencyScore.value = {
      ...agencyScore.value,
    ...
    // 중개사 리뷰 목록 조회
    const res_reviews = await axios.get(
      `http://54.66.153.95:8080/api/agent/reviews/${office_id}`
    );
