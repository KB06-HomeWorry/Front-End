...
onMounted(async () => {
  try {
    const res = await axios.get(
      `http://54.66.153.95:8080/api/member/getprofile/${token}`
    );
    const data = res.data;
    user.value = {
      ...user.value,
...
const onDeleteMouseEnter = () => {
  isDeleteHover.value = true;
};
...
const menuList = computed(() => [
  {
    icon: bookmarkIcon,
    label: "저장된 중개사무소 목록",
    onClick: goToAgencyBookmark,
  },
  {
    icon: listingbookmarkIcon,
    label: "저장된 매물 목록",
    onClick: goToListingBookmark,
  },
...
async function onPwModalSuccess() {
  if (deleteMode.value) {
    ...
    const pwResetToken = await axios.get(
      `http://54.66.153.95:8080/api/member/verify-password/${token}`
    );
    router.push(
      `/auth/change-password/${encodeURIComponent(pwResetToken.data)}`
    );
  }
...
const handleDeleteClickAfterPw = async () => {
  ...
  try {
    await axios.delete(`http://54.66.153.95:8080/api/member/withdraw/${token}`);
    alert("회원탈퇴가 완료되었습니다.");
